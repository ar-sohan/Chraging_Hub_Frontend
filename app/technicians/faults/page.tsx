...
import { getMyProfile, reportFault } from '../lib/api';
import { faultReportSchema } from '../lib/schemas';

const initialForm = { stationId: '', stationLocation: '', faultType: '', description: '', severity: 'Medium', reportDate: '' };
...
export default function FaultReportPage() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const result = faultReportSchema.safeParse(form);
    if (!result.success) {
      setMessage('');
    ...
    setMessage('');
    try {
      await reportFault(technicianId, result.data);
      setMessage('Fault report submitted successfully.');
      setForm(initialForm);
    } catch (requestError) {
      setError(getMessage(requestError, 'Could not submit the fault report'));
    } finally {
  ...
  return (
    <TechnicianAppLayout>
      <section className="mx-auto max-w-3xl px-5 py-12">
        <PageTitle title="Report / claim a fault" description="Record a fault found on a charging station or device." />
        <form noValidate onSubmit={handleSubmit} className="grid gap-4 rounded-lg border border-slate-200 bg-white p-6 shadow-sm md:grid-cols-2">
          <TextField label="Device / Station ID" name="stationId" value={form.stationId} onChange={updateField} placeholder="DEV-202" />
          <TextField label="Device / Station location" name="stationLocation" value={form.stationLocation} onChange={updateField} placeholder="Gulshan-2, Dhaka" />
          <TextField label="Fault type" name="faultType" value={form.faultType} onChange={updateField} placeholder="Charging port damaged" />
          <SelectField label="Severity" name="severity" value={form.severity} onChange={updateField} />
          <TextField label="Report date" name="reportDate" value={form.reportDate} onChange={updateField} placeholder="YYYY-MM-DD" />
          <TextField label="Description" name="description" value={form.description} onChange={updateField} placeholder="Describe the fault" />
